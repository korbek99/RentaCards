...
class ReservationViewController: UIViewController {
    ...
    private let confirmButton: UIButton = {
        ...
        button.setTitle("Confirmar Renta", for: .normal)
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 18)
        button.translatesAutoresizingMaskIntoConstraints = false
        button.addTarget(self, action: #selector(confirmRental), for: .touchUpInside)
        return button
    }()
    ...
    private func loadDatauser(){
        ...
        if let car = car {
            ...
            yearCarLabel.text = "Year : \(car.year)"
            priceCarLabel.text = "Price: $\(car.pricePerDay)"
        } else {
            makeLabel.text = "N/A"
            modelLabel.text = "N/A"
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            confirmButton.topAnchor.constraint(equalTo: endDatePicker.bottomAnchor, constant: 30),
    ...
    @objc private func confirmRental() {
      
        print("Confirmar Renta")
    }
}
